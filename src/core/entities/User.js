...
const { ResourceNotExists } = require('../../errors/resourceErrors')
...
class User {
  ...
  async createUser (name, nickname) {
    const user = new User(this.userID, this.email, name, nickname)
  ...
  static async getUser (userID) {
    return await performQuery(
      'user',
      async collection => {
        const query = await collection.findOne({ userID })
